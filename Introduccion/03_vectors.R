...
v4[c(1, 1, 4)] # obtenemos el primero doble y el cuarto
v4[m:n]        # obtenemos los elementos con los indices del m al n
v4[[7]] <- 6   # establecemos el valor 6 en el septimo elemento
v4[which(v == 'M')] # which devuelve los indices

# Indexando con numeros negativos (Estos no)
v4[-1] # todos menos el primer elemento
v4[-length(v)] # todos menos el ultimo elemento
v4[-c(1,3,5,7,9)] # todos menos esos indices

# Indexando con vectores logicos (atomicos)
v4[c(TRUE, FALSE, TRUE)] # obtenemos el 1 y el 3 elemento
v4[v4 > 2] # todos los elementos donde v > 2
v4[v4 > 2 & v < 9] # todos los elementos donde v>2 y v<9
v4[v4 == 'M'] # todos los que v sea igual a 'M'
v4[v4 %in% c('me', 'andMe', 'meToo')]

# Indexando por nombre (solo con vectores con nombre)
v4[['alpha']] # obtenemos todos los del mismo nombre
v4[['beta']] <- 'b' # establecemos el valor
v4[c('alpha', 'beta')] 
v4[!(names(v) %in% c('a','b'))] # todos los que no esten en a y b
names(v4)['z'] <- 'omega' # establecemos el nombre omega
