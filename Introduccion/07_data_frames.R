...
setwd("C:/Users/David/Documents/GitHub/R_BigData_Deportivo/Introduccion")
...
write.csv(df, file = "archivo_prueba.csv") #exportar
...
col.vec <- df$cats # devuelve un vector
col.vec <- df[, 'horses'] # devuelve un vector
col.vec <- df[, a] # a es un int o string
col.vec <- df[['frogs']] # devuelve un vector
frogs.df <- df['frogs'] # devueve un df de 1 columna
...
df <- rbind(df, data.frame(col1 = 'd', col2 = 3, col3 = 'A'))
...
df[, 'copyOfCol'] <- df$col # copiamos una columna
df$y.percent.of.x <- df$y /sum(df$x) * 100
df <- cbind(col, df)
df <- cbind(df, col)
df$c3 <- with(df, c1 + c2) # sin comillas
